...
void Polygons::SetPosition(float x, float y, float w, float h, float tx, float ty, float tw, float th) {
	/*頂点座標設定*/
	vertex[0].x = x;
	vertex[0].y = y;
	vertex[1].x = x + w;
...
void Polygons::SetColor(D3DCOLOR deffuse) {
	/*頂点色設定*/
	vertex[0].deffuse = deffuse;
	vertex[1].deffuse = deffuse;
	vertex[2].deffuse = deffuse;
	vertex[3].deffuse = deffuse;
}

void Polygons::Draw(LPDIRECT3DTEXTURE9 texture) {
